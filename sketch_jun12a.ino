...
void setup(){
  ...
  wifiMulti.addAP("hanserena", "1234567890"); // 将需要连接的一系列WiFi ID和密码输入这里
  wifiMulti.addAP("一个普通的WiFi", "asdfghjkl"); // ESP8266-NodeMCU再启动后会扫描当前网络
...
void handleLEDControl(){
   bool ledStatus = digitalRead(D5);     // 此变量用于储存LED状态     
   ledStatus == HIGH ? digitalWrite(D5, LOW) : digitalWrite(D5, HIGH);  // 点亮或者熄灭LED  
...
bool handleFileRead(String resource) {            //处理浏览器HTTP访问
  ...
  if (resource.endsWith("/")) {                   // 如果访问地址以"/"为结尾
    resource = "/LED.html";                     // 则将访问地址修改为/LED.html便于SPIFFS访问
  } 
